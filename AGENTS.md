...
### 近期模型掃臉穩定性修復（2026-03-17）

- 主流程一致化
  - `embed:batch` 與 `embed:references` 統一為「先檢查模型狀態，不就緒先嘗試 `preloadModel()` 再決定是否阻擋」。
- 掃描 session 隔離
  - 每次新掃描前清空本次記憶體 embedding map，避免混入上一個資料夾掃描結果。
- 快取來源一致性
  - `faces` table 新增 `source` 欄位（`face | deterministic | unknown`），保留 embedding 來源並在回讀時還原，避免比對懲罰邏輯不一致。
- 參考照狀態同步
  - 參考照被新增/拖放/替換時，強制 `refsLoaded=0`，要求重新載入，避免「UI 已換圖但仍用舊 embedding」。
  - UI 改為同時顯示「已選 N / 已載入 M」。
- 訊息語義分流
  - 將成功、警告、資訊、錯誤提示分開呈現，不再把非錯誤訊息統一用紅色錯誤框。
