...
export function getDb(): BetterSqlite3Database {
  ...
  // v5: Wipes embeddings generated with broken EXIF orientation handling and
  //     incorrect Umeyama SVD computation (v0.2.19 fix). All cached embeddings
  //     from v0.2.18 and earlier used wrong KPS coordinates for rotated photos
  //     (iPhone portrait mode, etc.), producing misaligned 112×112 faces and
  //     garbage ArcFace vectors. Must recompute with corrected pipeline.
  ...
  return rows.flatMap((r: FaceRow): FaceResult[] => {
    try {
      ...
      if (parsed.length !== 512) {
        logger.warn(
          `DB: embedding dimension mismatch for ${safePath}: expected 512, got ${parsed.length} — skipping stale cache entry`
        );
        return [];
      }
